// Fill out your copyright notice in the Description page of Project Settings.


#include "5week/MonsterBase.h"
#include "5week/QuestEventSubsystem.h"
#include "SpartaUnrealMasterCharacter.h"
#include "GameplayTagContainer.h"

// Sets default values
AMonsterBase::AMonsterBase()
{
 	// Set this character to call Tick() every frame.  You can turn this off to improve performance if you don't need it.
	PrimaryActorTick.bCanEverTick = true;

}

// Called when the game starts or when spawned
void AMonsterBase::BeginPlay()
{
	Super::BeginPlay();
	
}

// Called every frame
void AMonsterBase::Tick(float DeltaTime)
{
	Super::Tick(DeltaTime);

}

// Called to bind functionality to input
void AMonsterBase::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	Super::SetupPlayerInputComponent(PlayerInputComponent);

}

void AMonsterBase::Die(ASpartaUnrealMasterCharacter* Killer)
{
    UGameInstance* GameInstance = GetGameInstance();
    if (GameInstance)
    {
        UQuestEventSubsystem* QuestEventSubsystem = GameInstance->GetSubsystem<UQuestEventSubsystem>();
        if (QuestEventSubsystem)
        {
            AController* KillerController = Killer->GetController();
            QuestEventSubsystem->BroadcastQuestEvent(MonsterTag, KillerController, 1);
        }
    }
}