...
{
	if (!ItemIDToLoad.IsValid()) return;

	UAssetManager& AssetManager = UAssetManager::Get();

	TArray<FName> Bundles;

	LoadingHandle = AssetManager.LoadPrimaryAsset(ItemIDToLoad, Bundles,
		FStreamableDelegate::CreateUObject(this, &AMyDataAssetTestActor::OnLoadFinished, ItemIDToLoad)
	);
...
{
	LoadedItem = Cast<UMyTestPrimaryDataAsset>(UAssetManager::Get().GetPrimaryAssetObject(LoadedId));
